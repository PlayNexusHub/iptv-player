...
- 🌍 Smart country-aware playlist browser with live stats
- 🎯 Advanced channel filters, quality sorting, and geo-free toggles
- ⭐ Favorite channels for fast access and retry/copy tools
...
- Copy `env.example` to `.env` (never commit secrets) and mirror `config_template.json` for each tenant.
- Run `npm run release:prepare` to generate `release/PlayNexus_IPTV_vX.Y.Z` with documentation + obfuscated renderer bundle.
- Use `npm run build:secure` for Windows installers that include javascript-obfuscator output.
- Ensure the release folder always includes `README_PlayNexus.txt`, `README.md`, `CHANGELOG.txt`, `version.json`, `config_template.json`, `.env.example`, and `SETUP_INSTRUCTIONS.txt` before tagging/publishing.
